...
class Tab(wx.Panel):
	...
	def OnInsert(self, event): 
		...
		for val, text in self.textAreas.items():
			new_value = text.GetValue().strip()
			if val == '姓名':
				if self.df[val].str.match(new_value).any():
					dial = wx.MessageDialog(None, '該名字已出現在資料庫內，確定要繼續新增新資料?\n如要新增請按yes,若不要新增請按no', 'Question', 
	...
	def OnDelete(self, event): 
		...
		if not res.empty:
			for idx,val in res.iterrows():
				self.df = self.df.drop(self.df.index[idx-base])
				base += 1
